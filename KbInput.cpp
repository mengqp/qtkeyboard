...
//    DbConn.setDatabaseName("c:/pinyin.db");
    ...
    //如果需要更改输入法面板的显示位置,改变 currentPosition 这个变量即可
    ...
    //如果需要更改输入法初始模式,改变 currentType 这个变量即可
...
bool CKbInput::eventFilter(QObject *obj, QEvent *event)
{
    ...
    if (event->type() == QEvent::MouseButtonPress) {
        ...
        //Shift 切换输入法模式,esc 键关闭输入法面板,空格取第一个汉字,退格键删除
...
void CKbInput::btn_clicked()
{
    ...
    if (objectName == "btnType") {
        ...
        if (currentPY_index >= 20) {
            //每次最多显示 10 个汉字,所以每次向前的时候索引要减 20
        ...
        //如果是&按钮，因为对应&被过滤,所以真实的 text 为去除前面一个&字符
        if (objectName == "btnOther7") {
            value = value.right(1);
        }
        //当前不是中文模式,则单击按钮对应 text 为传递参数
        if (currentType != "chinese") {
            insertValue(value);
        } else {
...
void CKbInput::deleteValue()
{
    if (currentEditType == "QLineEdit") {
        ...
        //获取当前 QTextEdit 光标,如果光标有选中,则移除选中字符,否则删除光标前一个字符
